...
{
	// if we have a switch/case here, gcc uses data (which we can't have as we're in the Sun 3 PROM which is only accessible as SPROG)
#if 0
	switch (phase) {
...
#else
	return CSR_SDRAM_DFII_PI0_WRDATA_ADDR + 24*phase;
#endif
}
...
#else
	return CSR_SDRAM_DFII_PI0_RDDATA_ADDR + 24*phase;
#endif
}
...
static inline void init_sequence(uint32_t a32)
{
	/* Release reset */
